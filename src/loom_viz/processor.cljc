(ns ^{:doc "Process csv format to dot."
      :author "Jude Payne"}
  loom-viz.processor
  (:require [loom-viz.graph                 :as g]
            [loom.graph                     :as loom.graph]
            [loom.attr                      :as loom.attr]
            [extra-loom.multigraph          :as multigraph]
            [clojure.string                 :as str]
            [loom-viz.clustered             :as clstr]
            [loom-viz.preprocessor          :as preprocessor]
            [loom-viz.postprocessor         :as postprocessor]
  ...
  (if (and (some? (:paths opts)) (string? (:paths opts)))
    (let [subs (str/split (:paths opts) #"\|")]
      (if (= 2 (count subs))
        ...
        (throw (tb/err "Error: Paths input cannot be parsed."))))
    g))
...
(defn process-to-svg [in dot->svg]
  (case (:format-in in)
    ...
    (throw (tb/err "Error: only 'csv' or 'dot' are allowed input formats."))))
