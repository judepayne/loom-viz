(ns ^{:doc "Post process the svg."
      :author "Jude Payne"}
  loom-viz.postprocessor
  (:require [loom-viz.svg          :as svg]
            [loom-viz.anneal       :as anneal]
            [loom.graph            :as loom.graph]
            [loom-viz.clustered    :as clstr]
            [loom-viz.geometry     :as g]
...
(defn str->int [cs error-msg]
  (if (integer? cs)
    ...
    (try
      (parse-integer cs)
      #?(:clj (catch Exception e (throw (tb/err error-msg)))
         :cljs (catch js/Error e (throw (tb/err error-msg)))))))
